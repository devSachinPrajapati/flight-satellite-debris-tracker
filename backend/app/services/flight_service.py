"""
Flight Data Service - OPTIMIZED WITH NON-BLOCKING STARTUP
"""
import asyncio
...
class FlightService:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    params=params,
                    timeout=aiohttp.ClientTimeout(total=10)  # ⚡ Faster timeout
                ) as resp:
                    if resp.status == 200:
    ...
    def generate_mock_flights(self) -> List[dict]:
        """Generate realistic mock flight data - UNCHANGED"""
        airports = [
    ...
    def interpolate_position(self, flight_id: str) -> Optional[dict]:
        """Calculate interpolated position - UNCHANGED"""
        ...
        while True:
            try:
                ...
                if flights and not self.use_mock_data:
                    self.update_cache_from_api(flights)
                    mode = "REAL"
                    if self.api_call_count % 5 == 0:  # Log every 5 updates
                        print(f"✅ Updated {len(self.flights_cache)} {mode} flights")
                elif self.use_mock_data and self.api_call_count % 10 == 0:
                    # Refresh mock data occasionally
                    mock_flights = self.generate_mock_flights()
                    self.update_cache_from_api(mock_flights)
            except Exception as e:
