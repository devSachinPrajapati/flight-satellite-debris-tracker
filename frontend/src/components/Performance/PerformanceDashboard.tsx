...
const PerformanceDashboard = ({
  ...
  // ✅ FIX: Record data on every render when aircraft data updates
  useEffect(() => {
    recordPerformanceData(aircraft);
  }, [aircraft, recordPerformanceData]);

  // ✅ FIX: Update chart data whenever performance changes
  useEffect(() => {
    if (performance && performance.data_points.length > 0) {
      ...
      setChartData(data);
      console.log('📊 Updated chart data for', aircraft.hex, ':', data.length, 'points');
    } else {
      setChartData([]);
    }
  }, [performance, aircraft.hex]);

  // ✅ Show loading state while collecting initial data
  if (!performance || performance.data_points.length === 0) {
    return (
    ...
    <div className="fixed right-4 top-2 z-50 bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-[600px] max-h-[80vh] overflow-y-auto">
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="text-xs text-gray-500 dark:text-gray-400">
          Tracking {performance.data_points.length} data points for {aircraft.hex}
        </div>
      </div>
    </div>
  );
