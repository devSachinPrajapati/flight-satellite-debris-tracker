...
export const useSearch = (
  ...
  const handleSearch = useCallback((query: string, newFilters: FilterOptions) => {
  ...
  const filteredResults = useMemo(() => {
    ...
    // Apply search query
    if (searchQuery) {
      results.aircraft = results.aircraft.filter(ac =>
      ...
      results.satellites = results.satellites.filter(sat =>
      ...
      results.debris = results.debris.filter(deb =>
    ...
    if (filters.minAltitude !== undefined) {
      results.aircraft = results.aircraft.filter(ac => (ac.alt / 3.281) >= filters.minAltitude!);
      results.satellites = results.satellites.filter(sat => sat.altitude >= filters.minAltitude!);
      results.debris = results.debris.filter(deb => deb.altitude >= filters.minAltitude!);
    }

    if (filters.maxAltitude !== undefined) {
      results.aircraft = results.aircraft.filter(ac => (ac.alt / 3.281) <= filters.maxAltitude!);
      results.satellites = results.satellites.filter(sat => sat.altitude <= filters.maxAltitude!);
      results.debris = results.debris.filter(deb => deb.altitude <= filters.maxAltitude!);
    }

    // Operator filter
    if (filters.operator) {
      results.satellites = results.satellites.filter(sat => sat.operator === filters.operator);
      results.debris = results.debris.filter(deb => deb.operator === filters.operator);
    }

    // Speed filter
    if (filters.minSpeed !== undefined) {
      results.satellites = results.satellites.filter(sat => sat.velocity >= filters.minSpeed!);
      results.debris = results.debris.filter(deb => deb.velocity >= filters.minSpeed!);
    }
