...
//             Alt: {ac.alt ? `${Math.round(ac.alt / 3.281)} km` : "N/A"} • Speed:{" "}
//             {ac.speed || "N/A"} kts
//           </div>
//         </div>
//       </div>
...
//           </div>
//           <div className="text-xs text-gray-600 mt-1">
//             Alt: {sat.altitude.toFixed(0)} km • Vel: {sat.velocity.toFixed(2)}{" "}
//             km/s
//           </div>
//         </div>
//       </div>
...
//           <div className="text-xs text-gray-500 mt-1">ID: {deb.norad_id}</div>
//           <div className="text-xs text-gray-600 mt-1">
//             Alt: {deb.altitude.toFixed(0)} km •
//             {deb.conjunction_risk && (
...
const ObjectsList = ({
  ...
  const [filters, setFilters] = useState<any>({ objectType: "all" });
  ...
  const filteredData = useMemo(() => {
    ...
    // Filter aircraft (from viewport-filtered list)
    let aircraft = [...allAircraft];
    if (query) {
      aircraft = aircraft.filter(
        (ac) =>
          ac.flight_icao?.toLowerCase().includes(query) ||
          ac.hex.toLowerCase().includes(query) ||
          ac.airline_icao?.toLowerCase().includes(query) ||
          ac.aircraft_icao?.toLowerCase().includes(query)
      );
    }

    // Filter satellites (from viewport-filtered list)
    let satellites = [...allSatellites];
    if (query) {
      satellites = satellites.filter(
        (sat) =>
          sat.name.toLowerCase().includes(query) ||
          sat.norad_id.toLowerCase().includes(query) ||
          sat.operator?.toLowerCase().includes(query)
      );
    }

    // Filter debris (from viewport-filtered list)
    let debris = [...allDebris];
    if (query) {
      debris = debris.filter(
        (deb) =>
          deb.name.toLowerCase().includes(query) ||
          deb.norad_id.toLowerCase().includes(query)
      );
    }

    // Apply additional filters
    if (filters.minAltitude !== undefined) {
      aircraft = aircraft.filter((ac) => ac.alt / 3.281 >= filters.minAltitude);
      satellites = satellites.filter((sat) => sat.altitude >= filters.minAltitude);
      debris = debris.filter((deb) => deb.altitude >= filters.minAltitude);
    }

    if (filters.maxAltitude !== undefined) {
      aircraft = aircraft.filter((ac) => ac.alt / 3.281 <= filters.maxAltitude);
      satellites = satellites.filter((sat) => sat.altitude <= filters.maxAltitude);
      debris = debris.filter((deb) => deb.altitude <= filters.maxAltitude);
    }

    if (filters.operator) {
      satellites = satellites.filter((sat) => sat.operator === filters.operator);
      debris = debris.filter((deb) => deb.operator === filters.operator);
    }

    if (filters.minSpeed !== undefined) {
      satellites = satellites.filter((sat) => sat.velocity >= filters.minSpeed);
      debris = debris.filter((deb) => deb.velocity >= filters.minSpeed);
    }

    return { aircraft, satellites, debris };
  }, [allAircraft, allSatellites, allDebris, searchQuery, filters]);
  ...
  const currentData = useMemo(() => {
    if (selectedTab === "aircraft") return filteredData.aircraft;
    if (selectedTab === "satellites") return filteredData.satellites;
    return filteredData.debris;
  ...
  const renderItem = (index: number) => {
    if (selectedTab === "aircraft") {
      const ac = filteredData.aircraft[index];
      const isSelected = selectedObject?.data?.hex === ac.hex;
      ...
      const sat = filteredData.satellites[index];
      const isSelected = selectedObject?.data?.norad_id === sat.norad_id;
        ...
        >
          <div className="flex items-start space-x-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-600 mt-1">
                Alt: {sat.altitude.toFixed(0)} km • Vel: {sat.velocity.toFixed(2)} km/s
              </div>
            </div>
          </div>
        </div>
      ...
      const deb = filteredData.debris[index];
      const isSelected = selectedObject?.data?.norad_id === deb.norad_id;

      return (
        <div
          onClick={() => onSelectObject(deb, "debris")}
          className={`p-3 hover:bg-red-50 cursor-pointer border-b transition ${
            isSelected ? "bg-red-100 border-l-4 border-red-500" : ""
          }`}
        >
          <div className="flex items-start space-x-2">
            <Trash2 size={16} className="text-red-500 mt-1 shrink-0" />
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-600 mt-1">
                Alt: {deb.altitude.toFixed(0)} km •
                {deb.conjunction_risk && (
                  <span className="text-red-600 font-semibold ml-1">⚠️ Risk</span>
                )}
              </div>
            </div>
          </div>
      ...
      >
        <div className="h-full bg-white shadow-2xl flex">
          ...
          <div className="w-80 flex flex-col h-full">
            {/* Header */}
            <div className="p-4 border-b bg-gray-50">
              <h3 className="font-bold text-lg">Visible Objects</h3>
              <p className="text-xs text-gray-600 mt-1">
                {allAircraft.length + allSatellites.length + allDebris.length} objects on map
              </p>
            </div>
            ...
            <div className="flex border-b">
              ...
              >
                <div className="flex flex-col items-center space-y-1">
                  <Plane size={18} />
                  <span>Aircraft</span>
                  <span className="text-xs">({filteredData.aircraft.length})</span>
                </div>
              </button>
              ...
              >
                <div className="flex flex-col items-center space-y-1">
                  <Satellite size={18} />
                  <span>Satellites</span>
                  <span className="text-xs">({filteredData.satellites.length})</span>
                </div>
              </button>
              ...
              >
                <div className="flex flex-col items-center space-y-1">
                  <Trash2 size={18} />
                  <span>Debris</span>
                  <span className="text-xs">({filteredData.debris.length})</span>
                </div>
              </button>
            </div>
            ...
            <div
              ref={parentRef}
              className="flex-1 overflow-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200"
            >
              {currentData.length === 0 ? (
                <div className="p-4 text-center text-gray-500 text-sm">
                  {searchQuery ? "No matching objects" : "No objects on map"}
                </div>
              ) : (
                <div
                  style={{
                ...
                >
                  {rowVirtualizer.getVirtualItems().map((virtualItem: { key: Key | null | undefined; size: any; start: any; index: number; }) => (
                    <div
                      key={virtualItem.key}
                      style={{
                        position: "absolute",
